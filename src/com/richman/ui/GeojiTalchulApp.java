...
public class GeojiTalchulApp extends JFrame {
				...
				protected void done() {
					...
					try {
						...
						if (res != null && res.has("userId")) {
							...
							String msg = (res != null && res.has("message")) ? res.get("message").getAsString()
									: "아이디 또는 비밀번호가 일치하지 않습니다.";
							JOptionPane.showMessageDialog(GeojiTalchulApp.this, msg, "로그인 실패",
	...
	JPanel buildSignupPanel() {
		...
		JRadioButton m = new JRadioButton("남성", true);
		m.setOpaque(false);
		m.setFont(FONT);
		JRadioButton f = new JRadioButton("여성");
		f.setOpaque(false);
		f.setFont(FONT);
		...
		backBtn.addActionListener(e -> {
			...
			signupBirthField.setText("");
			signupPhoneField.setText("");
			signupJobField.setText("");
			signupAddrField.setText("");
			signupIncomeField.setText("");
			((CardLayout) rootContainer.getLayout()).show(rootContainer, "AUTH_LOGIN");
		});
		...
		submitBtn.addActionListener(e -> {
			...
			if (loginId.isEmpty() || password.isEmpty() || userName.isEmpty()) {
				JOptionPane.showMessageDialog(this, "아이디, 비밀번호, 이름은 필수 입력 항목입니다.", "입력 오류",
			...
			if (!incomeStr.isEmpty()) {
				try {
					income = Integer.parseInt(incomeStr.replace(",", ""));
			...
			new SwingWorker<String, Void>() {
				...
				protected String doInBackground() throws Exception {
					// 3. 아이디 중복 확인
					JsonObject dupRes = httpGet("http://localhost:8080/api/users/check-duplicate?loginId="
				...
				protected void done() {
					...
					try {
						...
						if (errorMsg == null) {
							...
							JOptionPane.showMessageDialog(GeojiTalchulApp.this, errorMsg, "회원가입 실패",
									JOptionPane.ERROR_MESSAGE);
						}
					} catch (Exception ex) {
						JOptionPane.showMessageDialog(GeojiTalchulApp.this,
	...
	JPanel buildSidebar() {
		...
		user.setBorder(new CompoundBorder(new MatteBorder(1, 0, 0, 0, BORDER), new EmptyBorder(14, 16, 14, 16)));
		// 기본 프로필인 거지 이미지로 적용
	...
	void showUserInfoEditDialog() {
		...
		Image currentImg = currentIcon.getImage().getScaledInstance(-1, 342, Image.SCALE_SMOOTH);
		...
		changePicBtn.addActionListener(e -> {
			JOptionPane.showMessageDialog(dlg, "프로필 사진 변경 기능은 갤러리 API 연동 후 지원됩니다.", "알림",
	...
	void showNotifications() {
		String msg = state.budgetUsage() >= 1.0 ? "예산을 초과했습니다.\n지출 내역을 확인해 보세요."
	...
	void showSettings() {
		...
		save.addActionListener(e -> {
			...
			try {
				...
				dlg.dispose();
				refreshAll();
				JOptionPane.showMessageDialog(this, "목표 예산이 " + won(newBudget) + "으로 설정되었습니다.", "설정 완료",
						JOptionPane.INFORMATION_MESSAGE);
			} catch (NumberFormatException ex) {
	...
	class HomePanel extends JPanel {
		...
		JPanel buildAlertBanner() {
			...
			detail.addActionListener(e -> {
				if (state.budgetUsage() >= 1.0 || state.budgetUsage() >= state.alertThreshold / 100.0) {
					showCard("STATS");
				} else if (state.fixedExpenseCandidateCount() > 0
		...
		void showFixedExpenseCandidateDialog() {
			...
			for (Expense e : candidates) {
				...
				row.setBorder(new CompoundBorder(new MatteBorder(0, 0, 1, 0, BORDER), new EmptyBorder(14, 8, 14, 8)));
		...
		JPanel buildBudgetCard() {
			...
			// 🌟 3. 거지 찰진 랜덤 대사 목록
			String[] quotes = { "오늘 점심은 삼각김밥이다...", "문성도원", "피들스정", "숨만 쉬어도 돈이 나가네...", "노르톨트 후버", "이러다간 진짜 길바닥 나앉아!" };

			// 말풍선 사라지는 타이머를 담을 변수
			final javax.swing.Timer[] hideTimer = { null };

			// 🌟 4. 캐릭터 클릭 시 대사 띄우기
			// 사진 주변의 빈 여백까지 클릭되는 것을 막기 위해 판정을 약간 줄였습니다.
		...
		void refresh() {
			...
			List<Expense> list = state.expenses.stream().sorted(Comparator.comparing((Expense x) -> x.date).reversed())
					.limit(6).collect(Collectors.toList());
			for (Expense e : list)
				recentList.add(expenseRow(e));
		...
		JPanel expenseRow(Expense e) {
			...
			row.setBackground(WHITE);
			row.setBorder(new CompoundBorder(new MatteBorder(0, 0, 1, 0, new Color(242, 244, 246)),
					new EmptyBorder(12, 22, 12, 22)));

			JLabel date = new JLabel(e.date.format(DateTimeFormatter.ofPattern("MM. dd (E)", Locale.KOREAN)));
			...
			JLabel item = new JLabel(e.item == null || e.item.trim().isEmpty() ? "(내용 없음)" : e.item);
	...
	class CalendarPanel extends JPanel {
		...
		void showDateDetail(LocalDate date) {
			...
			for (Expense x : items) {
				...
				row.setBorder(new CompoundBorder(new MatteBorder(0, 0, 1, 0, BORDER), new EmptyBorder(12, 8, 12, 8)));
				...
				JLabel categoryLabel = new JLabel(esc(x.large) + " > " + esc(x.medium));
				categoryLabel.setFont(new Font("Malgun Gothic", Font.PLAIN, 12));
	...
	class StatisticsPanel extends JPanel {
		...
		StatisticsPanel() {
			...
			filters[0] = "전체 지출 내역";
			for (int i = 0; i < state.largeCategories.size(); i++)
				filters[i + 1] = state.largeCategories.get(i);
			...
			detailTable.getColumnModel().getColumn(4).setCellRenderer(new javax.swing.table.DefaultTableCellRenderer() {
				...
				public Component getTableCellRendererComponent(JTable table, Object value, boolean selected,
					...
					JLabel label = (JLabel) super.getTableCellRendererComponent(table, value, selected, focused, row,
							column);
		...
		void rebuildExpenseTable() {
			...
			List<Expense> list = state.expenses.stream()
					...
					.sorted(Comparator.comparing((Expense x) -> x.date).reversed()).collect(Collectors.toList());

			for (Expense e : list) {
				tableModel
						.addRow(new Object[] { e.date.format(DateTimeFormatter.ofPattern("MM. dd (E)", Locale.KOREAN)),
								e.large, e.medium, e.small, "-" + won(e.amount) });
			}
			if (list.isEmpty()) {
	...
	class PieChart extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			Color[] colors = { GREEN, BLUE, ORANGE, PURPLE, new Color(90, 150, 130), new Color(205, 128, 120),
					new Color(150, 150, 95) };
	...
	class TrendChart extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			for (int i = 0; i < 6; i++) {
				...
				vals[i] = state.expenses.stream().filter(e -> YearMonth.from(e.date).equals(ym))
	...
	class CommunityPanel extends JPanel {
		...
		JPanel buildRanking() {
			...
			head.add(rule, BorderLayout.EAST);
			...
			list.setCellRenderer(new DefaultListCellRenderer() {
				...
				public Component getListCellRendererComponent(JList<?> list, Object value, int index,
					...
					if (isSelected) {
						label.setBorder(new CompoundBorder(BorderFactory.createLineBorder(BLUE, 2),
		...
		JPanel buildFeed() {
			...
			head.add(title, BorderLayout.WEST);
			head.add(write, BorderLayout.EAST);
			p.add(head, BorderLayout.NORTH);
		...
		void createChallenge() {
			...
			p.add(new JLabel("챌린지 이름"));
			p.add(name);
			p.add(new JLabel("목표 지출금액"));
			p.add(goal);
			p.add(new JLabel("보상 포인트"));
			p.add(reward);
			int r = JOptionPane.showConfirmDialog(this, p, "그룹 챌린지 생성", JOptionPane.OK_CANCEL_OPTION);
			if (r == JOptionPane.OK_OPTION) {
				challengeModel.addElement(" " + name.getText() + "   | 목표 " + goal.getText() + "원 | 보상 "
						+ reward.getText() + "P | 참여 1명");
				state.points += 50;
				refreshAll();
			}
		}
		...
		private void showWritePostDialog() {
			...
			textArea.addFocusListener(new FocusAdapter() {
				public void focusGained(FocusEvent e) {
					if (textArea.getText().equals("오늘의 지출 내역이나 다짐을 공유해 보세요!"))
						textArea.setText("");
				}
			});
	...
	class MyStorePanel extends JPanel {
		...
		JPanel buildStore() {
			...
			head.add(title, BorderLayout.WEST);
			head.add(myPoints, BorderLayout.EAST);
			p.add(head, BorderLayout.NORTH);
			...
			addSkinShopItem(items, " 프로거지 스킨", "캐릭터 꾸미기", "20,000P", 20000, "richman.png");
	...
	void openExpenseDialog(String presetLarge, String presetMedium, LocalDate presetDate) {
		...
		if (presetLarge != null)
			large.setSelectedItem(presetLarge);
		JComboBox<String> medium = new JComboBox<>();
		Runnable updateMedium = () -> {
			medium.removeAllItems();
			String l = (String) large.getSelectedItem();
			for (String m : state.mediumMap.getOrDefault(l, new ArrayList<>()))
				medium.addItem(m);
			if (presetMedium != null && Objects.equals(l, presetLarge))
				medium.setSelectedItem(presetMedium);
		};
		...
		save.addActionListener(e -> {
			try {
				...
				if (s.trim().isEmpty())
					throw new IllegalArgumentException("소분류를 입력하세요.");
				...
				if (state.budgetUsage() >= 1)
					JOptionPane.showMessageDialog(this, "지출이 저장되었습니다.\n\n⚠ 예산을 초과했습니다.", "예산 경고",
							JOptionPane.WARNING_MESSAGE);
				else
					JOptionPane.showMessageDialog(this, "지출이 저장되었습니다. +20P", "저장 완료", JOptionPane.INFORMATION_MESSAGE);
			} catch (Exception ex) {
	...
	static class AppState {
		...
		List<String> largeCategories = Arrays.asList("주거/통신", "금융/보험", "정기구독", "식비", "교통/차량", "생활/쇼핑", "취미/여가",
				"경조사/선물", "의료/건강", "유지/수리");
		...
		double budgetUsage() {
			return budget == 0 ? 0 : totalSpent() / (double) budget;
		}

		Map<String, Long> largeTotals() {
			return expenses.stream().filter(e -> YearMonth.from(e.date).equals(YearMonth.of(2026, 8))).collect(
					Collectors.groupingBy(e -> e.large, LinkedHashMap::new, Collectors.summingLong(e -> e.amount)));
		}
		...
		List<Expense> fixedCandidates() {
			Map<String, List<Expense>> map = expenses.stream()
					...
					.collect(Collectors.groupingBy(e -> e.medium + "|" + e.amount, LinkedHashMap::new,
							Collectors.toList()));
			return map.values().stream().filter(v -> v.size() >= 3).map(v -> v.get(0)).collect(Collectors.toList());
		}

		int fixedExpenseCandidateCount() {
			return fixedCandidates().size();
		}

		long fixedCandidateRepeatCount(Expense candidate) {
			return expenses.stream()
					.filter(e -> e.date.isAfter(LocalDate.of(2026, 5, 31)) && e.date.isBefore(LocalDate.of(2026, 9, 1)))
					.filter(e -> e.medium.equals(candidate.medium) && e.amount == candidate.amount).count();
		}

		String fixedCandidateKey() {
			return fixedCandidates().stream().map(e -> String.valueOf(e.id)).sorted().collect(Collectors.joining(","));
		}
	}

	static class Expense {
		int id;
		String large, medium, small, item;
		long amount;
		LocalDate date;
		boolean fixed;

		Expense(int id, String large, String medium, String small, String item, long amount, LocalDate date,
				boolean fixed) {
			this.id = id;
			this.large = large;
			this.medium = medium;
	...
	String skinDisplayName(String skinFile) {
		if ("poorman.png".equals(skinFile))
			return "거지 스킨";
		if ("richman.png".equals(skinFile))
			return "프로 거지 스킨";
		return skinFile;
	}
	...
	static class CircleBorder extends AbstractBorder {
		...
		CircleBorder(Color color, int diameter) {
			this.color = color;
